...
function App() {
  ...
  const fetchPeers = useCallback(async () => {
    ...
    } catch (e) { console.error("Peer sync failed"); }
  }, []);
  ...
  // 4. Global Status Monitor (Internet + Hardware)
  useEffect(() => {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if(data.status === "sent") {
        setMessages(prev => [...prev, { text: inputText, sender: "Me", via: data.mode === "Internet" ? "🌐 Cloud" : "📡 LoRa" }]);
        setInputText("");
      }
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', fontFamily: 'Segoe UI', backgroundColor: '#f0f2f5' }}>
      ...
      {/* Sidebar Section */}
      <div style={{ width: '310px', background: '#1c2938', color: 'white', padding: '25px', display: 'flex', flexDirection: 'column', boxShadow: '2px 0 10px rgba(0,0,0,0.1)' }}>
        <h2 style={{ textAlign: 'center', color: '#3498db', marginBottom: '25px', letterSpacing: '1px' }}>Hybrid Console</h2>
        ...
        <div style={{ padding: '20px', background: connectedUser ? '#27ae60' : '#2c3e50', borderRadius: '12px', textAlign: 'center', marginBottom: '20px' }}>
          ...
          <div style={{ fontSize: '0.7rem', marginTop: '5px', opacity: 0.8 }}>{connectedUser ? `Hardware: ${connectedUser}` : "Waiting for T3S3..."}</div>
        </div>

        <h4 style={{ fontSize: '0.85rem', marginBottom: '10px', color: '#3498db' }}>TARGET PEER</h4>
        <select value={selectedPeer} onChange={(e) => setSelectedPeer(e.target.value)} style={{ width: '100%', padding: '12px', borderRadius: '8px', background: '#34495e', color: 'white', border: 'none', cursor: 'pointer' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
        ...
        {/* Header with Fixed Identity Logic */}
        <header style={{ background: 'white', padding: '18px 35px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid #ddd' }}>
          <div style={{ fontWeight: 'bold', fontSize: '1.2rem', color: '#2c3e50' }}>Communication Hub</div>
          ...
          <div style={{ textAlign: 'right' }}>
            <div style={{ color: isOnline ? '#2ecc71' : '#e67e22', fontWeight: 'bold', display: 'flex', alignItems: 'center', gap: '10px', justifyContent: 'flex-end' }}>
              ...
              {/* DISPLAY HARDWARE NAME/ID IN BOTH MODES */}
              {connectedUser || "Device Offline"}
            </div>
            <small style={{ fontSize: '0.7rem', color: '#95a5a6' }}>
              Identity: {isOnline ? "🌐 Global Mesh" : "📡 Local Radio"}
            </small>
          </div>
        </header>

        <div style={{ flex: 1, padding: '30px', overflowY: 'auto', display: 'flex', flexDirection: 'column', background: '#ffffff' }}>
          {messages.length === 0 && (
              <div style={{ textAlign: 'center', color: '#bdc3c7', marginTop: '15%' }}>
                <div style={{ fontSize: '3.5rem', marginBottom: '15px' }}>📡</div>
                <p>System ready for transmission.</p>
              </div>
          )}
          {messages.map((m, i) => (
            <div key={i} style={{ alignSelf: m.sender === "Me" ? 'flex-end' : 'flex-start', maxWidth: '65%', marginBottom: '15px', padding: '14px 22px', background: m.sender === "Me" ? '#0084ff' : '#f1f3f4', color: m.sender === "Me" ? 'white' : '#1c2938', borderRadius: '22px' }}>
              <small style={{ display: 'block', fontWeight: 'bold', fontSize: '0.7rem', marginBottom: '5px', opacity: 0.8 }}>{m.sender === "Me" ? "YOU" : m.sender} • {m.via}</small>
              <div style={{ fontSize: '1.05rem', lineHeight: '1.5' }}>{m.text}</div>
            </div>
          ))}
        ...
        <div style={{ padding: '25px 35px', background: 'white', borderTop: '1px solid #eee', display: 'flex', gap: '15px' }}>
          ...
          <input 
            ...
            disabled={!connectedUser || isLoading} 
            style={{ flex: 1, padding: '15px 25px', borderRadius: '35px', border: '1px solid #e0e0e0', outline: 'none' }} 
            placeholder={isOnline ? "Message via Internet..." : "Message via LoRa..."}
          />
          <button 
            onClick={sendMessage} 
            disabled={!connectedUser || isLoading} 
            style={{ background: '#0084ff', color: 'white', border: 'none', padding: '0 40px', borderRadius: '35px', fontWeight: 'bold', minWidth: '120px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
          >
